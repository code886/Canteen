...
<div id="MainArea">
    <table class="MainArea_Content" cellspacing="0" cellpadding="0">
		...
		<!--显示数据列表 -->
        <tbody id="TableData">
		<c:forEach var="table" items="${requestScope.list }" varStatus="vs">
			<tr class="TableDetail">
				<td align="center">${vs.count} </td>
				<td align="center">${table.tableName}&nbsp; </td>
				<c:choose>
					<c:when test="${table.tableStatus==1 }">
						<td align="center">已订</td>
					</c:when>
					<c:otherwise>
				...
				<td>
					<c:choose>
						<c:when test="${table.tableStatus==1 }">
							<a href="${pageContext.request.contextPath }/table?method=update&id=${table.id}" class="FunctionButton">
								退桌
							</a>
						</c:when>
						<c:otherwise>
					...
					<a href="${pageContext.request.contextPath }/table?method=delete&id=${table.id}" 
